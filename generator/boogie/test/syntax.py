...
class TestMapVariableAccess(unittest.TestCase):
    ...
    def test_recursive_map_access(self):
        ...
        self.assertEqual(MapVariableAccess(map_access, map_key), f"{base_map_var}[{base_map_key}][{map_key}]")
...
class TestFunction(unittest.TestCase):
    ...
    def test_function_without_parameters_nor_axioms(self):
        self.assertEqual(Function(self.func_name, self.func_return_type), f"function {self.func_name}() "
                                                                          f"returns ({self.func_return_type});\n")

    def test_function_without_parameters(self):
        self.assertEqual(Function(self.func_name, self.func_return_type, axioms=self.axioms),
                         f"function {self.func_name}() returns ({self.func_return_type});\n" + "".join(
                             list(map(str, self.axioms))))

    def test_function(self):
        self.assertEqual(
            Function(self.func_name, self.func_return_type, axioms=self.axioms, parameters=self.parameters),
...
class TestProcedure(unittest.TestCase):
    ...
    def test_procedure_empty_procedure(self):
        self.assertEqual(str(Procedure(self.procedure_name)),
                         f"procedure {self.procedure_name}();\nimplementation {self.procedure_name}() {{\n}}\n")

    def test_procedure_with_return_var(self):
        self.assertEqual(Procedure(self.procedure_name, self.procedure_return_variable),
    ...
    def test_procedure_with_parameters(self):
        ...
        self.assertEqual(Procedure(self.procedure_name, parameters=self.parameters),
    ...
    def test_procedure_with_variable_declarations(self):
        ...
        self.assertEqual(Procedure(self.procedure_name, variable_declarations=self.variable_declarations),
                         f"procedure {self.procedure_name}();\n"
    ...
    def test_full_procedure(self):
        ...
        self.assertEqual(Procedure(self.procedure_name, variable_declarations=self.variable_declarations,
                                   return_var=self.procedure_return_variable, instructions=self.instructions,
