...
implementation Sum() {
  var aRef: Ref;
  var bRef: Ref;
	aRef := Stack[StackPointer];
	bRef := Stack[StackPointer-1];
  call Pop();
  call Pop();
...
implementation AppOptedIn() {
    ...
    applicationIDRef := Stack[StackPointer];
    accountsIndexRef := Stack[StackPointer-1];
		call Pop();
		call Pop();
...
implementation contract() {
	...
	check_equals:
		...
		call Int(arg);
		call Equal();
		if (RefToInt[Stack[StackPointer]] == 0) {
			call Pop();
			goto add_1;
		}
		call Pop();
		assert RefToInt[Stack[StackPointer]] < 2;
		return;
	add_1:
		call Int(1);
		call Sum();
