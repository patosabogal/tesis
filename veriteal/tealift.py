...
class Instruction:
    def __init__(self, operation: str, arguments: List[str] = [], consumes: List[int] = []):
    ...
    def from_dict(cls, instruction_dict: InstructionDict):
        return cls(instruction_dict['op'], instruction_dict['args'], instruction_dict['consumes'])
...
class BasicBlock:
    ...
    def from_dict(cls, dictionary: BasicBlockDict):
        instructions = map(lambda instruction: Instruction.from_dict(instruction), dictionary['instructions'])
        return cls(dictionary['incoming_edges'],
...
class Tealift:
    ...
    def from_dict(cls, dictionary: TealiftDict):
        basic_blocks = map(lambda bb: BasicBlock.from_dict(bb), dictionary['basic_blocks'])
            ...
            if phi_values[incoming_edges_index] >= 0:
                ...
                res.extend(
                    self._get_phi_instructions(
                        incoming_edge,
                        self.basic_blocks[incoming_edge].phis[abs(phi_values[incoming_edges_index]) - 1]
                    )
                )
        return res
