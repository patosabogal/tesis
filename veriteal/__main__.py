    ...
    def declare_transaction_fields(transaction_index):
        ...
        for array_field in transaction_array_fields:
            for array_index in range(max_array_slots):
                global_variables += int_variable_declaration(
                    transaction_array_variable_name(
...
def max_variables_values(methods: List[Method]) -> Tuple[int, int, int, int, int]:
    ...
    max_applications = min(
        max(max_applications, applications), FOREIGNS_MAX_COMBINED_SIZE
    )
...
def main_contract_procedure(tealift: Tealift):
    ...
    for block_index, block in enumerate(tealift.basic_blocks):
        ...
        for instruction_index, instruction in enumerate(block.instructions):
            try:
                ...
                parsed_instruction: ParsedInstruction = instruction_class(
                    instruction, instruction_index, block, block_index
                )
            ...
            if parsed_instruction.returns_value():
                ...
                parsed_instruction_boogie = variable_assigment(
                    var_name, parsed_instruction_boogie
                )
    ...
    for slot in range(SCRATCH_SLOTS):
        procedure_variables_declaration += int_variable_declaration(
            scratch_slot_variable_name(slot)
        )
    ...
    with open("veriteal/artifacts/tealift_output.json", "w+") as tealift_json:
        subprocess.run(
            [
                ...
                "-y",
                "ts-node",
                "algorand-tealift/tealift/src/print.ts",
                teal_file_path,
            ],
            stdout=tealift_json,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Set recursion depth bound. Defaults to 1000.",
    )
    parser.add_argument(
        "-c",
        "--custom",
        help="Flag indicating that the JSON uses the custom VeriTeal schema." "Schema.",
        action="store_true",
    )
    ...
    interface_json_path = (
        args.interface_json
        if args.interface_json
        else f"{os.path.splitext(args.teal_file)[0]}_interface.json"
    )
    assertions_json_path = (
        args.assertions_json
        if args.assertions_json
        else f"{os.path.splitext(args.teal_file)[0]}_assertions.json"
    )
    ...
    if not args.custom:
        ...
        methods = list(
            map(
                lambda method: Method.from_abi_method(
                    method, hints[method.get_signature()]
                ),
        ...
        methods += list(
            map(
                lambda tuple: Method.from_bare_call(tuple[0], tuple[1]),
        ...
        if not has_constructor:
            raise Exception(
                "Constructor method missing from interface schema. Please provide constructor interaface."
            )
    ...
    subprocess.run(
        [
            "corral",
            "veriteal/artifacts/output.bpl",
            f"/main:{VERIFY_PROCEDURE}",
            f"/recursionBound:{args.recursionBound}",
        ]
    )
